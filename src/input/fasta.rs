...
pub struct RevCompFastaReader {
    buffer: Box<dyn BufRead>,
    ty: Val<'static>,
}

impl RevCompFastaReader {
    pub fn new(buffer: Box<dyn BufRead>) -> RevCompFastaReader {
        let ty = Val::RecTy {
            fields: vec![
                CoreRecField {
                    name: b"seq",
        ...
        RevCompFastaReader { buffer, ty }
    }
}

impl Reader for RevCompFastaReader {
    fn map<'p>(
        &mut self,
        prog: &core::Prog<'p>,
        env: &Env<Val<'p>>,

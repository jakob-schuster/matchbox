...
/// Global configuration options related to input of reads.
#[derive(Args)]
struct InputReads {
...
/// Global configuration option including a read file and an optional paired file.
#[derive(Args, Clone)]
...
/// Global configuration options to take in matchbox scripts
#[derive(Args)]
...
fn run(code: &str, global_config: &GlobalConfig) {
    ...
    // create the standard library environment
    let env = ctx.tms;
...
impl GenericError {
    /// Pretty prints the error with codespan, terminating execution
